...
{
	return mipi_dsih_read_part(fbi, MIPI_CMD_PKD_STATUS, MIPI_GEN_CMD_FULL, 1);
}
...
{
	return mipi_dsih_read_part(fbi, MIPI_CMD_PKD_STATUS, MIPI_GEN_PLD_W_FULL, 1);
}
...
{
	return mipi_dsih_read_part(fbi, MIPI_CMD_PKD_STATUS, MIPI_GEN_RD_CMD_BUSY, 1);
}
...
{
	return mipi_dsih_read_part(fbi, MIPI_CMD_PKD_STATUS, MIPI_GEN_PLD_R_EMPTY, 1);
}
 ...
 */
static unsigned short mipi_dsih_gen_rd_packet(struct comipfb_info *fbi, uint8_t vc, uint8_t data_type, uint8_t msb_byte, uint8_t lsb_byte, uint8_t bytes_to_read, uint8_t* read_buffer)
{
	int err_code = 0;
	int timeout = 0;
	int counter = 0;
	...
	if ((!mipi->commandmode_info.ack_rqst_en) && (!mipi->commandmode_info.ack_rqst_en) && (!mipi->videomode_info.frame_bta_ack))
	...
	if (!(timeout < DSIH_FIFO_ACTIVE_WAIT)) {
		printf( "mipi read, tx timeout");
		if ((!mipi->commandmode_info.ack_rqst_en) && (!mipi->commandmode_info.ack_rqst_en) 
				&& (!mipi->videomode_info.frame_bta_ack))
 ...
 * @note this function will enable BTA
 */
unsigned short mipi_dsih_dcs_rd_cmd(struct comipfb_info *fbi, uint8_t vc, uint8_t command, uint8_t bytes_to_read, uint8_t* read_buffer)
{
 ...
 * @note this function will enable BTA
 */
unsigned short mipi_dsih_gen_rd_cmd(struct comipfb_info *fbi, uint8_t vc, uint8_t* params, uint16_t param_length, uint8_t bytes_to_read, uint8_t* read_buffer)
{
	uint8_t data_type = 0;
...
int mipi_dsih_gen_wr_packet(struct comipfb_info *fbi, uint8_t vc, uint8_t data_type, uint8_t* params, uint16_t param_length)
{
	int err_code = 0;
...
int mipi_dsih_dcs_wr_cmd(struct comipfb_info *fbi, uint8_t vc, uint8_t* params, uint16_t param_length)
{
	...
	switch (params[0]) {
		...
		break;
		case SHUTDOWN_SW_PACK:
			packet_type = 0x22;
		default:
			printf( "NOT DCS COMMAND\n");
		return -EINVAL;
	}
