...
#define MFP_BASE 	0xA1003000
#define GPIO192_MPF	(MFP_BASE + 0x24)
...
void gpio192_out(int level)
{
	unsigned int reg;
	unsigned int bit;
	unsigned int val;
	unsigned int offset = 192-128;

	reg = GPIO1_BASE + GPIO_PORT_DR(offset);
	bit = offset % 16;
	val = (1 << (bit + 16));
	if(level)
		val |= 1 << bit;
	...
	reg = GPIO1_BASE + GPIO_PORT_DDR(offset);
	bit = offset % 16;
	val = (1 << (bit + 16)) | (1 << bit);
	__raw_writel(val, reg);
}

void gpio192_init(void)
{
	unsigned long value = 0;
	value = __raw_readl(GPIO192_MPF);
	value &= ~(3<<28);
	value |= 2<<28;
	__raw_writel(value, GPIO192_MPF);

	gpio192_out(0);
}


void gpio192_notify(void)
{
	int m=10;
	while(m--){
		gpio192_out(1);
...
void memory_test(void)
{
	...
	u32 memsize, vstart, memsize_b;
	ulv* vstart_2;
	vstart = PHYS_SDRAM_1;
	memsize = PHYS_SDRAM_1_SIZE;

	//	WATCH_GPIO_INIT();
	//	WATCH_GPIO_NOTIFY();
	vstart_2 = (ulv*)(vstart + PHYS_SDRAM_1_SIZE / 2);
	memsize_b = PHYS_SDRAM_1_SIZE /(2 * sizeof(u32));
